...
function exportSheetToJson(luckysheet,name) { // 参数为luckysheet.getluckysheetfile()获取的对象
    ...
    downloadElement.download = name+".txt"; // 文件名字
    document.body.appendChild(downloadElement);
    downloadElement.click();
    document.body.removeChild(downloadElement); // 下载完成移除元素
    window.URL.revokeObjectURL(href); // 释放掉blob对象
}
function exportSheetExcelToSave(luckysheet,name) { // 参数为luckysheet.getluckysheetfile()获取的对象
    // 1.创建工作簿，可以为工作簿添加属性
    const workbook = new ExcelJS.Workbook();
    // 2.创建表格，第二个参数可以配置创建什么样的工作表
    // 2.创建表格，第二个参数可以配置创建什么样的工作表
    ...
    const buffer = workbook.xlsx.writeBuffer().then(data => {
        ...
        var fd = new FormData();
        fd.append('fname', 'test.xlsx');
        fd.append('files', blob);
        $.ajax({
            type: 'POST',
            url: 'http://192.168.0.110:8081/api/excel/upload',
            data: fd,
            processData: false,
            contentType: false
        }).done(function(data) {
